...
RID = "EXP-20260902-001-virtual-wrist-segment-contact-scan"
ROOT = Path(__file__).resolve().parents[2]
CFG = ROOT / "research/configs/EXP-20260902-001-virtual-wrist-segment-contact-scan.json"
SCENE = ROOT / "research/sources/mujoco_menagerie_robotstudio_so101/robotstudio_so101/scene.xml"
SOURCE = ROOT / "research/artifacts/EXP-20260901-002-virtual-wrist-contact-gate/contact-coverage.json"
...
def main() -> None:
    ...
    end = source["first_maximum_zero_contact"]
    start = next(row for row in source["rows"] if row["sample_index"] == 0)
    rec = RunRecorder(
        run_id=RID,
        ...
        inputs=[
            ...
            {"kind": "source_contact_table", "path": SOURCE.relative_to(ROOT).as_posix(), "sha256": file_hash(SOURCE)},
            {"kind": "model", "path": SCENE.relative_to(ROOT).as_posix(), "sha256": file_hash(SCENE), "revision": "da76818e269b82289eba39808e2fb91d679d6994"},
        ],
        baseline={"kind": "comparison", "reference": "EXP-20260901-002 records only static sampled poses, not intermediate configurations."},
        level="formal",
    )
    rec.__enter__()
    ...
    alphas = np.linspace(0.0, 1.0, cfg["protocol"]["segment_samples"])
    rows = []
    ...
    meta = {
        ...
        "source_endpoint_visible_cups": end["visible_cups"], "cups": source["cups"], "rows": rows,
    }
