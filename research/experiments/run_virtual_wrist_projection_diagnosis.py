...
RID = "EXP-20260916-001-virtual-wrist-projection-diagnosis"
ROOT = Path(__file__).resolve().parents[2]
CFG = ROOT / "research/configs/EXP-20260916-001-virtual-wrist-projection-diagnosis.json"
...
def main() -> None:
    ...
    rec = RunRecorder(
        run_id=RID,
        objective="Record candidate-camera center projection conditions without attributing raster absence or asserting hardware visibility.",
        ...
        baseline={"kind": "comparison", "reference": "EXP-20260902-003 reports ray/raster disagreement without camera-frame diagnosis."},
        level="formal",
    )
    with rec:
        ...
        for source_row in segment["rows"]:
            for cup_index, position in enumerate(segment["cups"]):
                ...
                camera_coordinates = world_to_camera @ (data.geom_xpos[cup_id] - camera_position)
                ...
                rows.append({
                    ...
                    "world_center": data.geom_xpos[cup_id].tolist(), "camera_position": camera_position.tolist(),
                    "camera_coordinates": camera_coordinates.tolist(), "forward_depth": forward_depth,
                    "behind_camera": behind, "outside_candidate_depth_clip": depth_clipped,
        ...
        rec.complete(
            passed=True,
            criteria="All source configuration-cup pairs were diagnosed and artifacts written; no visibility or capability criterion.",
