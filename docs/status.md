...
`EXP-20260902-003-virtual-wrist-raster-visibility` 对上述 650 个配置—杯位组合比较几何射线和
腕部虚拟相机 segmentation：40 个组合射线命中杯体，但没有组合出现杯体 raster 像素。该候选模型
观察保持 `pending_human_review`；尚未区分投影、裁剪、姿态或场景设置，也不代表实体腕部 RGB 覆盖。
...
先冻结六个核心协议的 v0 草案和行为测试；随后在新 run 中只对候选腕部相机的单个中心视锥内/raster
不可见组合记录完整杯体几何边界与渲染诊断，并保持它和真机标定分离。MRBTP-demo 已完成上游
exploratory 自检；任何外部项目在与本项目协议的正式证据建立前不得集成进产品源码。
